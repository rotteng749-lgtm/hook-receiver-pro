...
/**
 * Create a short link via ShrtFly and store the result in the database.
 * Action (not mutation) because it performs an external fetch.
 */
export const createShortLink = action({
  ...
  handler: async (ctx, args) => {
    // Actions have a different ctx type — verify auth via an internal query
    const auth = (await ctx.runQuery(internal.nameserver.debugAuth, {})) as {
    ...
    if (auth.role !== "owner" && auth.role !== "admin") {
      throw new Error(
        `Forbidden — your role is "${auth.role}" but this action requires: owner or admin`,
      );
    }
    ...
    const doc = await ctx.runQuery(internal.nameserver.getSettingsInternal, {});
    const apiKey = doc.shortenerApiKey || "";
    if (!apiKey) {
    ...
    const data = (await res.json()) as
      | { status: "success"; result: { original_url: string; shorten_url: string; stats_url?: string } }
    ...
    const record = await ctx.runMutation(internal.nameserver.insertShortLink, {
