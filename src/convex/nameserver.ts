...
export const updateSettings = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const shortenerApiKey = (args.shortenerApiKey ?? "").trim().slice(0, 128);
    const shortenerAdType =
...
export const createCustomEndpoint = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const RESERVED = ["health", "connect", "auth", "files", "databases", "api", "mod", "telegram", "hook", "getkey", "s", "PUBGM"];
...
/** Owner: update just the ShrtFly shortener credentials (Shortener page). */
export const updateShortenerSettings = mutation({
  args: {
    shortenerApiKey: v.string(),
    shortenerAdType: v.optional(v.number()),
  },
  handler: async (ctx, args) => {
    await requireRole(ctx, ["owner"]);
    const patch = {
      shortenerApiKey: args.shortenerApiKey.trim().slice(0, 128),
      shortenerAdType: args.shortenerAdType === 2 ? 2 : 1,
    };
    const doc = await getSettingsDoc(ctx);
    if (doc) {
      await ctx.db.patch(doc._id, patch);
...
/**
 * Issue a trial key through the token-based /getkey endpoint.
 *
 * Identity is the API token (sha256 hash) — apiTokens rows are not linked to
 * users, so the daily quota is tracked per token per UTC day in `getkeyDaily`.
 * The key lasts settings.getkeyHours hours (default 5) and the issuer may call
 * this settings.getkeyMaxPerDay times per day (default 3).
 *
 * Fully atomic: quota check, key generation and usage bookkeeping happen in
 * one mutation so concurrent calls can't exceed the daily limit.
 */
export const issueGetkey = internalMutation({
  args: { tokenHash: v.string() },
  handler: async (ctx, { tokenHash }) => {
    const doc = await getSettingsDoc(ctx);
    const hours = doc?.getkeyHours ?? DEFAULT_SETTINGS.getkeyHours;
    const maxPerDay = doc?.getkeyMaxPerDay ?? DEFAULT_SETTINGS.getkeyMaxPerDay;
    const price = doc?.getkeyPrice ?? DEFAULT_SETTINGS.getkeyPrice;
    ...
    const usage = await ctx.db
      .query("getkeyDaily")
      .withIndex("by_token_day", (q) => q.eq("tokenHash", tokenHash).eq("day", day))
      .first();
    const used = usage?.count ?? 0;
    if (used >= maxPerDay) {
      return { ok: false as const, reason: "daily_limit", used, maxPerDay, hours };
    }

    // Preferred server from settings, else the first active one.
    let server = doc?.getkeyServerId ? await ctx.db.get(doc.getkeyServerId) : null;
    ...
    if (server === null) {
      return { ok: false as const, reason: "no_server", used, maxPerDay, hours };
    }
    ...
    const owner = await ctx.db
      ...
      .first();
    if (owner === null) {
      return { ok: false as const, reason: "no_owner", used, maxPerDay, hours };
    }
    ...
    if (usage) {
      await ctx.db.patch(usage._id, {
        count: used + 1,
        spentCoins: usage.spentCoins + price,
        keyIds: [...(usage.keyIds ?? []), keyId],
      });
    } else {
      await ctx.db.insert("getkeyDaily", {
        tokenHash,
        day,
        count: 1,
        spentCoins: price,
...
/** Current GetKey quota for a token (used by GET /getkey without ?generate=1). */
export const getGetkeyUsage = internalQuery({
  args: { tokenHash: v.string() },
  handler: async (ctx, { tokenHash }) => {
    const doc = await getSettingsDoc(ctx);
    const hours = doc?.getkeyHours ?? DEFAULT_SETTINGS.getkeyHours;
    ...
    const usage = await ctx.db
      ...
      .first();
    const used = usage?.count ?? 0;
    return { used, maxPerDay, remaining: Math.max(0, maxPerDay - used), hours, day };
  },
});

/** Today's GetKey usage per token — shown on the panel's GetKey page. */
export const listGetkeyUsage = query({
  args: {},
  handler: async (ctx) => {
    await requireRole(ctx, ["owner", "admin"]);
    const doc = await getSettingsDoc(ctx);
    ...
    const rows = await ctx.db
      ...
      .collect();
    const tokens = await ctx.db.query("apiTokens").order("desc").take(200);
    const labels = new Map(tokens.map((t) => [t.tokenHash, t.label]));
    const maxPerDay = doc?.getkeyMaxPerDay ?? DEFAULT_SETTINGS.getkeyMaxPerDay;
    return {
      day,
      hours: doc?.getkeyHours ?? DEFAULT_SETTINGS.getkeyHours,
      price: doc?.getkeyPrice ?? DEFAULT_SETTINGS.getkeyPrice,
      maxPerDay,
      getkeyServerId: doc?.getkeyServerId ?? null,
      rows: rows.map((r) => ({
        _id: r._id,
        tokenLabel: labels.get(r.tokenHash) ?? "unknown token",
        count: r.count,
        spentCoins: r.spentCoins,
        remaining: Math.max(0, maxPerDay - r.count),
      })),
    };
  },
});

/** Owner/admin: mint one trial key now (same shape as /getkey, no quota). */
export const mintTrialKey = mutation({
  ...
  handler: async (ctx, { serverId }) => {
    ...
    if (server === null) {
      throw new Error("No active server — create one in Servers first");
    }
    ...
    const id = await ctx.db.insert("connectKeys", {
      ...
      uses: 0,
      expiresAt,
      cost: doc?.getkeyPrice ?? DEFAULT_SETTINGS.getkeyPrice,
      note: "getkey trial",
      maxDevices: 1,
    });
    return {
...
/** Owner: clear today's GetKey counters (one row, or all of today's). */
...
/** Persist a short link (called internally from the createShortLink action). */
export const insertShortLink = internalMutation({
  args: {
    alias: v.string(),
...
export const listShortLinks = query({
  args: {},
  handler: async (ctx) => {
    await requireRole(ctx, ["owner", "admin"]);
    return ctx.db.query("shortLinks").order("desc").take(200);
  },
});

/** Delete a short link (does not remove it from ShrtFly). */
export const deleteShortLink = mutation({
...
export const listDevices = query({
  ...
  handler: async (ctx) => {
    await requireRole(ctx, ["owner", "admin"]);
    return ctx.db.query("devices").order("desc").take(200);
  },
});
